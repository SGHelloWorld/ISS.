...
color_names=['orange red','yellow','lime','royal blue','dark violet'] 
...
def astronauts_in_ISS():
    ...
    for j in astronaut_names:
        ...
        ISS.forward(15)
        ISS.color(color_names[k])
        ISS.write(j,align='left',font=('Freestyle Script',18,'bold'))
        k=k+1
    display_message()   
    window.listen()
    window.onkey(main_menu,'m')
...
def main_menu(): # function to display the main menu
    ...
    menu.color('white')
    menu.goto(0,60)
    menu.write('~~~ Explore Space with Soham ~~~',align='center',font=('Freestyle Script',24,'bold'))
    menu.goto(-140,30)
